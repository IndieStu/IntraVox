OC.L10N.register(
    "intravox",
    {
    "PDF" : "PDF",
    "Spreadsheets" : "ئېلېكترونلۇق جەدۋەل",
    "Presentations" : "لېكسىيە",
    "Text" : "تېكىست",
    "Forms" : "جەدۋەل",
    "Other" : "باشقىلىرى",
    "Heading" : "ماۋزۇ",
    "Image" : "رەسىم",
    "Link" : "ئۇلىنىش",
    "File" : "ھۆججەت",
    "Video" : "سىن",
    "People" : "كىشىلەر",
    "Draft" : "لايىھە",
    "Scheduled" : "پىلانلانغان",
    "Expired" : "ۋاقتى توشتى",
    "Published" : "ئېلان قىلىندى",
    "Page deleted" : "بەت ئۆچۈرۈلدى",
    "Version restored" : "نەشرى ئەسلىگە كەلتۈرۈلدى",
    "Skip to main content" : "ئاساسلىق مەزمۇنغا ئاتلاڭ",
    "Unlock" : "قۇلۇپ ئېچىش",
    "Cancel editing" : "تەھرىرلەشنى ئەمەلدىن قالدۇرۇڭ",
    "Cancel" : "Cancel",
    "Save changes" : "ئۆزگەرتىشلەرنى ساقلاڭ",
    "Save" : "Save",
    "Loading …" : "يۈكلەۋاتىدۇ ...",
    "Details" : "تەپسىلاتى",
    "Untitled page" : "نامسىز بەت",
    "Learn more" : "تېخىمۇ كۆپ بىلىڭ",
    "Configured" : "تەڭشەلدى",
    "Invalid URL" : "ئىناۋەتسىز URL",
    "Deletion failed" : "ئۆچۈرۈش مەغلۇب بولدى",
    "Languages" : "تىللار",
    "Sharing" : "ھەمبەھىرلەش",
    "Support" : "قوللاش",
    "Recommended" : "تەۋسىيە قىلىندى",
    "Loading" : "يۈكلەۋاتىدۇ",
    "Language" : "تىل",
    "Content" : "مەزمۇن",
    "Status" : "ھالەت",
    "Action" : "ھەرىكەت",
    "Installing …" : "قاچىلاۋاتىدۇ  ...",
    "Install" : "قاچىلا",
    "Not available" : "ئىشلەتكىلى بولمايدۇ",
    "This action cannot be undone." : "بۇ ھەرىكەتنى بىكار قىلغىلى بولمايدۇ.",
    "Remove" : "ئۆچۈرۈڭ",
    "Visit website" : "توربېكەتنى زىيارەت قىلىڭ",
    "Add" : "قوش",
    "Saving …" : "ساقلاۋاتىدۇ  ...",
    "Import" : "ئەكىر",
    "Export" : "ئېكسپورت",
    "Select language" : "تىل تاللاڭ",
    "Empty" : "قۇرۇق",
    "Format" : "فورماتى",
    "Start import" : "ئىمپورت قىلىشنى باشلاڭ",
    "Comments" : "باھا",
    "Scope" : "دائىرىسى",
    "Path" : "Path",
    "Created" : "قۇرۇلدى",
    "Expires" : "ۋاقتى توشىدۇ",
    "Never" : "ھەرگىز",
    "Open in Files" : "ھۆججەتلەرنى ئېچىڭ",
    "Size" : "چوڭلۇقى",
    "Last modified" : "ئاخىرقى قېتىم ئۆزگەرتىلگەن",
    "Actions" : "ھەرىكەتلەر",
    "Recover" : "ئەسلىگە كەلتۈرۈش",
    "Delete" : "Delete",
    "Active" : "ئاكتىپ",
    "Name" : "ئاتى",
    "Type" : "تىپ",
    "Base URL" : "ئاساسى URL",
    "Advanced options" : "ئىلغار تاللاشلار",
    "Header name" : "ماۋزۇ نامى",
    "Value" : "قىممەت",
    "Authentication" : "دەلىللەش",
    "API token" : "API بەلگىسى",
    "Tenant ID" : "ئىجارە ئالغۇچى كىملىكى",
    "Client ID" : "خېرىدار ID",
    "Client secret" : "خېرىدار مەخپىيەتلىكى",
    "_%n item_::_%n items_" : ["%n تۈر","%n تۈرلەر"],
    "Deleting …" : "ئۆچۈرۋاتىدۇ ...",
    "Delete permanently" : "مەڭگۈلۈك ئۆچۈرۈڭ",
    "_%n comment_::_%n comments_" : ["%n ئىنكاسلار","%n ئىنكاسلار"],
    "Today" : "بۈگۈن",
    "Tomorrow" : "ئەتە",
    "No upcoming events" : "پات ئارىدا يۈز بېرىدىغان ئىشلار يوق",
    "All day" : "پۈتۈن كۈن",
    "Background color" : "تەگلىك رەڭگى",
    "None" : "يوق",
    "Light" : "نۇر",
    "Primary" : "بىرلەمچى",
    "Calendars" : "كالېندار",
    "read-only" : "پەقەت ئوقۇشقىلا بولىدۇ",
    "Date range" : "چېسلا دائىرىسى",
    "This week" : "بۇ ھەپتە",
    "This month" : "بۇ ئاي",
    "Next year" : "كېلەر يىلى",
    "RSS feed" : "RSS يىمىكى",
    "All languages" : "بارلىق تىللار",
    "Copy URL" : "Url نى كۆچۈرۈڭ",
    "Revoke" : "بىكار قىلىش",
    "Regenerate" : "قايتا ھاسىل",
    "Save settings" : "تەڭشەكلەرنى ساقلاڭ",
    "Newest first" : "ئەڭ يېڭى",
    "Oldest first" : "ئەڭ كونا",
    "Feed URL" : "URL نى بېقىڭ",
    "Disconnect" : "ئۈزۈڭ",
    "Overdue" : "ۋاقتى ئۆتۈپ كەتتى",
    "Project" : "تۈر",
    "Documents" : "ھۆججەتلەر",
    "List" : "تىزىملىك",
    "Layout" : "قۇرۇلما",
    "Grid" : "تور",
    "Columns" : "ستون",
    "Sort by" : "تەرتىپلەش",
    "Date" : "چېسلا",
    "Title" : "ماۋزۇ",
    "Preview" : "ئالدىن كۆرۈش",
    "Failed to load documents" : "ھۆججەتلەرنى يۈكلەش مەغلۇپ بولدى",
    "Retry" : "قايتا سىناڭ",
    "Open {name}" : "{name} نى ئېچىڭ",
    "Previous page" : "ئالدىنقى بەت",
    "Next page" : "كېيىنكى بەت",
    "Timeline" : "ۋاقىت جەدۋىلى",
    "Small" : "كىچىك",
    "Medium" : "ئوتتۇراھال",
    "Large" : "چوڭ",
    "Date modified" : "چېسلا ئۆزگەرتىلدى",
    "Filename" : "ھۆججەت ئىسمى",
    "Descending" : "تۆۋەنلەش",
    "Smallest first" : "ئەڭ كىچىك",
    "Ascending" : "ئۆرلەۋاتىدۇ",
    "contains" : "بار",
    "Source" : "مەنبە",
    "Filters" : "سۈزگۈچ",
    "Field" : "مەيدان",
    "Operator" : "ئەمەل",
    "Year" : "يىل",
    "Checked" : "تەكشۈرۈلدى",
    "Remove filter" : "سۈزگۈچنى ئۆچۈرۈڭ",
    "Add filter" : "سۈزگۈچ قوش",
    "Group by" : "بىلەن گۇرۇپپىلاش",
    "Order" : "زاكاز",
    "Display" : "كۆرسىتىش",
    "All" : "ھەممىسى",
    "Align left" : "سولغا توغرىلاڭ",
    "Align center" : "مەركەزگە توغرىلاڭ",
    "Align right" : "ئوڭغا توغرىلاڭ",
    "More options" : "تېخىمۇ كۆپ تاللاشلار",
    "Strikethrough" : "سىزىۋەت",
    "Bullet list" : "ئوق تىزىملىكى",
    "Numbered list" : "نومۇرلانغان تىزىملىك",
    "Blockquote" : "Blockquote",
    "Add row" : "قۇر قوشۇڭ",
    "Add column" : "ستون قوشۇڭ",
    "Delete table" : "جەدۋەلنى ئۆچۈرۈڭ",
    "Text alignment" : "تېكىستنى توغرىلاش",
    "Insert link" : "ئۇلىنىش قىستۇر",
    "Table" : "جەدۋەل",
    "Add row below" : "ئاستىغا قۇر قوش",
    "Width" : "كەڭلىك",
    "Auto" : "ئاپتوماتىك",
    "Delete row" : "قۇرنى ئۆچۈرۈڭ",
    "Delete column" : "ستوننى ئۆچۈرۈڭ",
    "URL:" : "URL:",
    "Remove link" : "ئۇلىنىشنى ئۆچۈرۈڭ",
    "Apply" : "ئىلتىماس قىلىڭ",
    "Home" : "ئۆي",
    "Information" : "ئۇچۇر",
    "Email" : "تورخەت",
    "Phone" : "تېلېفون",
    "Website" : "تور بېكەت",
    "Document" : "پۈتۈك",
    "Download" : "چۈشۈرۈش",
    "Help" : "ياردەم",
    "Settings" : "تەڭشەكلەر",
    "Account" : "ھېسابات",
    "Team" : "ئەتىرەت",
    "Calendar" : "كالېندار",
    "Analytics" : "ئانالىز",
    "Business" : "سودا",
    "Education" : "مائارىپ",
    "Favorite" : "ئامراق",
    "Like" : "Like",
    "Archive" : "ئارخىپ",
    "Manager" : "باشقۇرغۇچى",
    "Security" : "بىخەتەرلىك",
    "Admin" : "باشقۇرغۇچى",
    "Organization" : "ئورگان",
    "News" : "خەۋەرلەر",
    "Forum" : "مۇنبەر",
    "Code" : "كود",
    "Folder" : "قىسقۇچ",
    "Folders" : "ھۆججەت قىسقۇچ",
    "Chat" : "پاراڭ",
    "Message" : "ئۇچۇر",
    "Form" : "فورما",
    "Tasks" : "ۋەزىپە",
    "Dashboard" : "باش تاختا",
    "Photos" : "سۈرەتلەر",
    "Draw" : "سىزىش",
    "Poll" : "راي سىناش",
    "Appointment" : "تەيىنلەش",
    "Apps" : "ئەپلەر",
    "Info" : "ئۇچۇر",
    "Contacts" : "ئالاقىداشلار",
    "Add link" : "ئۇلىنىش قوشۇڭ",
    "Drag to reorder" : "قايتا تەرتىپلەش ئۈچۈن سۆرەڭ",
    "Title:" : "ماۋزۇ:",
    "Click here" : "بۇ يەرنى چېكىڭ",
    "or" : "ياكى",
    "Select an icon" : "سىنبەلگە تاللاڭ",
    "Default" : "كۆڭۈلدىكى",
    "Uploading …" : "چىقىرىۋاتىدۇ ...",
    "Upload" : "يۈكلەش",
    "Select" : "تاللا",
    "Remove file" : "ھۆججەتنى چىقىرىۋېتىش",
    "File already exists" : "ھۆججەت مەۋجۇت",
    "Overwrite" : "ئۆزگەرت",
    "Collapse" : "يىمىرىلىش",
    "Expand" : "كېڭەيتىش",
    "Item title" : "تۈر ئىسمى",
    "Custom URL" : "ئىختىيارى URL",
    "Blank page" : "بوش بەت",
    "Create" : "قۇر",
    "Carousel" : "كورسۇل",
    "Off" : "ئىتىك",
    "optional" : "ئىختىيارى",
    "Filter operator" : "سۈزگۈچ مەشغۇلاتچىسى",
    "Select value" : "قىممەتنى تاللاڭ",
    "Page settings" : "بەت تەڭشىكى",
    "New page" : "يېڭى بەت",
    "Delete page" : "بەتنى ئۆچۈرۈڭ",
    "Now" : "ھازىر",
    "Modified" : "ئۆزگەرتكەن",
    "Location" : "ئورنى",
    "Versions" : "نەشرى",
    "Current version" : "نۆۋەتتىكى نەشرى",
    "Version" : "نەشرى",
    "Restore this version" : "بۇ نەشرىنى ئەسلىگە كەلتۈرۈڭ",
    "Restore version" : "نەشرىنى ئەسلىگە كەلتۈرۈش",
    "Restore" : "ئەسلىگە كەلتۈرۈش",
    "Header row" : "بەت بېشى",
    "Duplicate row" : "قۇرنى كۆپەيت",
    "Column {column}" : "ستون {column}",
    "Disabled" : "چەكلەنگەن",
    "Disabled by administrator" : "باشقۇرغۇچىڭىز تەرىپىدىن تاقالغان",
    "Current" : "نۆۋەتتىكى",
    "Move up" : "يۆتكەڭ",
    "Move down" : "تۆۋەنگە يۆتكەڭ",
    "Rename" : "ئىسىم ئۆزگەرتىش",
    "Done" : "پۈتتى",
    "Search pages" : "ئىزدەش بەتلىرى",
    "Clear search" : "ئىزدەشنى تازىلاش",
    "Clear selection" : "تاللاشنى تازىلاش",
    "Show more" : "تېخىمۇ كۆپ كۆرسەت",
    "Group" : "گۇرۇپپا",
    "Organisation" : "تەشكىلات",
    "Role" : "رولى",
    "Select group" : "گۇرۇپپا تاللاڭ",
    "Days" : "كۈنلەر",
    "Basic information" : "ئاساسىي ئۇچۇرلار",
    "Avatar" : "باش سۈرەت",
    "Pronouns" : "ئاتىلىشى",
    "Headline" : "ماۋزۇ",
    "Contact" : "ئالاقىلىشىڭ",
    "Address" : "ئادرېس",
    "Date of birth" : "تۇغۇلغان ۋاقتى",
    "Photo" : "سۈرەت",
    "Start slideshow" : "تام تەسۋىرنى باشلاڭ",
    "Slideshow speed" : "تام تەسۋىر سۈرئىتى",
    "Normal" : "نورمال",
    "Show in Files" : "ھۆججەتلەردە كۆرسىتىڭ",
    "Close details" : "تەپسىلاتىنى ياپ",
    "Camera" : "كامېرا",
    "Jump to {year}" : "{year} ئاتلاڭ",
    "On this day" : "بۇ كۈنى",
    "Password required" : "پارول لازىم",
    "Password" : "پارول",
    "Template name" : "قېلىپ ئىسمى",
    "Description" : "چۈشەندۈرۈش",
    "Searching …" : "ئىزدەۋاتىدۇ ...",
    "No results found" : "ھېچقانداق نەتىجە تېپىلمىدى",
    "Public link" : "ئاممىۋى ئۇلىنىش",
    "Link copied to clipboard" : "ئۇلىنىش چاپلاش تاختىسىغا كۆچۈرۈلدى",
    "Password protected" : "پارول قوغدالدى",
    "Copy public link" : "ئاممىۋى ئۇلىنىشنى كۆچۈرۈڭ",
    "Total" : "ئومۇمىي",
    "Subscription key" : "مۇشتەرىلىك ئاچقۇچى",
    "Simple" : "ئاددىي",
    "Advanced" : "ئىلغار",
    "No users found" : "ئىشلەتكۈچى تېپىلمىدى",
    "No users selected" : "ئىشلەتكۈچى تاللانمىدى",
    "Domain" : "دائىرە",
    "Size:" : "چوڭلۇقى:",
    "Full width" : "تولۇق كەڭلىكى",
    "Width in pixels" : "پىكسېلنىڭ كەڭلىكى",
    "Top" : "يۇقۇرى",
    "Bottom" : "تۆۋەن",
    "Link:" : "ئۇلىنىش:",
    "External URL" : "سىرتقى URL",
    "Open in new tab" : "يېڭى بەتكۈچتە ئېچىڭ",
    "Local file" : "يەرلىك ھۆججەت",
    "Muted" : "ئاۋازسىز",
    "Preview:" : "ئالدىن كۆرۈش:",
    "Previous slide" : "ئالدىنقى تام تەسۋىر",
    "Next slide" : "كېيىنكى تام تەسۋىر",
    "_%n minute ago_::_%n minutes ago_" : ["%n مىنۇت ئىلگىرى","%n مىنۇت ئىلگىرى"],
    "_%n hour ago_::_%n hours ago_" : ["%n سائەت ئىلگىرى","%n سائەت ئىلگىرى"],
    "_%n day ago_::_%n days ago_" : ["%n كۈن ئىلگىرى","%n كۈن ئىلگىرى"],
    "edited" : "تەھرىرلەندى",
    "Edit" : "تەھرىر",
    "Reply" : "جاۋاب",
    "Add reaction" : "رېئاكسىيە قوشۇش",
    "Write a comment …" : "بىر ئىنكاس يېزىڭ  ...",
    "Send" : "ئەۋەتىڭ",
    "You and {count} others reacted with {emoji}" : "سىز ھەمدە باشقا {count} ئادەم {emoji} بىلەن ئىنكاس قايتۇردى",
    "{count} people reacted with {emoji}" : "{count} ئادەم {emoji} بىلەن ئىنكاس قايتۇردى"
},
"nplurals=2; plural=(n != 1);");
