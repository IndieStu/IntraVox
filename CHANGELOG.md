...
- **Page buttons for the File Story widget** ([#78](https://github.com/nextcloud/IntraVox/issues/78), thanks @kma-cloud). Setting a maximum number of documents used to hide everything past that count, with no way to reach older files. The widget editor now has a **Long lists** choice: keep the existing *Infinite scroll*, or switch to *Page buttons* with a **Documents per page** size and Previous/Next buttons that page through the rest, so every document stays reachable and the widget keeps a predictable height. **Maximum documents** stays a separate, optional total cap that applies in both modes. Page buttons apply to the List and Tiles layouts; the bucketed Timeline and Grouped layouts always use infinite scroll. Existing widgets are unchanged (they default to infinite scroll).
